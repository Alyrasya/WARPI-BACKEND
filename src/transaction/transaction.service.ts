...
export class TransactionService {
  ...
  async countTotalMonthlyIncome() {
    try {
      ...
      const result = await this.transactionRepository
        ...
        .where('transaction.payment_status = :payment_status', {
          payment_status: 'paid',
        })
  ...
  async getAllTransaction(
    ...
    if (method_name) {
      query.andWhere('paymentMethod.method_name = :method_name', {
        method_name,
      });
    }
    ...
    if (start_date && end_date) {
      const formattedStartDate =
        new Date(start_date).toISOString().split('T')[0] + 'T00:00:00.000+07';
      const formattedEndDate =
        new Date(end_date).toISOString().split('T')[0] + 'T23:59:59.999+07';
  ...
  async getByIdTransaction(id: string) {
    try {
      ...
      console.error(
        'Kesalahan saat mengambil transaksi oleh ID:',
        error.message,
      );
      throw new HttpException(
  ...
  //Customer
  async getAllTransactionsUser(id_user: string): Promise<Transaction[]> {
    return await this.transactionRepository.find({
      where: [{ customer: { id: id_user } }],
    ...
    const totalPriceTransaction = cart.order.reduce((total, order) => {
      ...
      if (isNaN(price)) {
        throw new BadRequestException(
          `Total price order tidak valid: ${order.total_price_order}`,
        );
      }
      return total + price;
    }, 0);
  ...
  async editTransaction(
    ...
    if (paymentMethod.method_name === 'cash') {
      ...
      if (cash < (transaction.total_price_transaction ?? 0)) {
        throw new Error(
          'Cash amount must be equal to or greater than the total price of the transaction',
        );
      }
      transaction.cash = cash;
      transaction.change_money = parseFloat(
        (cash - (transaction.total_price_transaction ?? 0)).toFixed(2),
      );
    ...
    transaction.payment_status =
      action === 'paid' ? PaymentStatus.Paid : PaymentStatus.Pending;
  ...
  //Cashier
  async getTransactionById(id_transaction: string): Promise<Transaction> {
  ...
  async getByIdDetail(id: string) {
    try {
      ...
      // Periksa payment_status dan sesuaikan data yang ditampilkan
      if (
