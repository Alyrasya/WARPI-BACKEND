...
export class CategoryService {
  ...
  async editCategory(id: string, data: UpdateCategoryDto): Promise<Category> {
    try {
      ...
      // Periksa apakah ada perubahan pada nama kategori
      if (
      ...
      // Periksa apakah ada perubahan pada status kategori
      if (
      ...
      // Jika tidak ada perubahan, kembalikan kategori tanpa menyimpan
      if (!isUpdated) {
        return category;
      }
      ...
      // Jika nama kategori diperbarui, perbarui juga di produk terkait
      if (category_name !== undefined) {
        await this.updateProductsCategoryName(
          updatedCategory.id,
  ...
  private async updateProductsCategoryName(
    id_category: string,
    ...
    try {
      ...
      console.error('Error saat memperbarui nama kategori di produk:', error);
      throw new Error(
        `Gagal memperbarui nama kategori di produk terkait: ${error.message}`,
      );
    }
  }
